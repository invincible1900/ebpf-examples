...
{
	const proc_info_t *e = data;
	printf("%s, %s, %s, %d, %d, %d, %d, %d, %d\n", e->comm, e->cmdline, e->filepath,
			e->parent_pid, e->parent_tgid,
			e->child_pid, e->child_tgid,
			e->uid, e->gid);
...
int main(int argc, char **argv)
{
	...
	if (signal(SIGINT, sig_int) == SIG_ERR) {
        ...
        rb = ring_buffer__new(bpf_map__fd(skel->maps.ringbuf_proc), handle_event_cb, NULL, NULL);
